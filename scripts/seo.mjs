...
const { posts, devPosts, researchNotes } = await import(join(root, 'dist-server', 'entry-server.js'))
...
// 매매일지 + 개발일지를 한 피드에 최신순으로
const feedPosts = [...posts, ...devPosts].sort((a, b) => (a.date < b.date ? 1 : -1))
const postUrl = (p) => abs(`${p.kind === 'dev' ? '/dev' : '/posts'}/${p.slug}`)
...
<rss version="2.0">
  <channel>
    ...
    <link>${HOME}</link>
    <description>공포와 욕심 사이를 자이로드롭처럼 오간 어느 개미의 회고록 — 매매일지와 개발일지</description>
