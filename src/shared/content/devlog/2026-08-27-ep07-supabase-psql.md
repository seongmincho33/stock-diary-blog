...
lang: ko
categories: [개발일지]
description: "Supabase CLI 사용법 정리 — login·link·start·status, migration new/up/list, db diff·pull·push --dry-run·reset, db query, inspect db. psql 설치(libpq)와 접속 3종, 백슬래시 메타 명령어, RLS 우회 함정."
---
...
## 수파 상회가 빌려주는 것 — Supabase 구성(Postgres·Auth·Storage·API)
...
## 전령 부르기 — Supabase CLI 설치·login·link
...
## 작업실 지하에 거울 서고 짓기 — supabase start와 로컬 포트
...
## 개축 기록 — 마이그레이션(db diff·pull·push·reset)
...
## 서고의 함정 네 가지 — RLS 우회, 스키마, 드리프트, 풀러
